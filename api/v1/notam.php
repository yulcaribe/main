...
function notamMapCacheDir(): string {
    $dir = rtrim(sys_get_temp_dir(), DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . 'yulcaribe_notam_map_v1';
    if (!is_dir($dir)) @mkdir($dir, 0770, true);
    return $dir;
}

function notamMapSyncVersion(PDO $pdo, string $environment): string {
    $stmt = $pdo->prepare(
...
function notamMapCacheRead(string $path, int $maxAgeSeconds = 900): ?array {
...
function notamMapCacheWrite(string $path, array $payload): void {
...
if ($action === 'map') {
    ...
    foreach (['west','south','east','north'] as $key) {
        if (!isset($_GET[$key]) || !is_numeric($_GET[$key])) {
            ycApiV1Respond(400, ['ok'=>false,'error'=>'Eksik/geçersiz bbox: '.$key]);
        }
    }
    ...
    if ($zoom < 4) {
        ycApiV1Respond(200, ['ok'=>true,'zoom'=>$zoom,'data'=>['type'=>'FeatureCollection','features'=>[]],'counts'=>['notam'=>0],'total'=>0,'truncated'=>false]);
    }
    $layers=['notam'];
    $features=[];
    $counts=['notam'=>0];
...
if (in_array('notam', $layers, true) && $zoom >= 4) {
    ...
    try {
        ...
        ycApiV1Respond(400, ['ok' => false, 'error' => 'Geçersiz NOTAM zamanı. UTC ISO tarih/saat gönder.']);
    }
    ...
    if (count($layers) === 1 && $layers[0] === 'notam') {
        $cacheVersion = notamMapSyncVersion($pdo, 'production');
        ...
        $cachedPayload = notamMapCacheRead($notamCacheFile, 900);
        $cacheReadMs = (microtime(true) - $cacheReadStart) * 1000.0;
        if ($cachedPayload !== null) {
            $totalMs = (microtime(true) - $notamPerfStart) * 1000.0;
            header('X-YC-Notam-Map-Cache: HIT');
            header('Server-Timing: notam-cache;dur=' . round($cacheReadMs, 1) . ', notam-total;dur=' . round($totalMs, 1));
            header('X-YC-Notam-Map-Timing: cache=' . round($cacheReadMs, 1) . 'ms,total=' . round($totalMs, 1) . 'ms');
            ycApiV1Respond(200, $cachedPayload);
        }

        header('X-YC-Notam-Map-Cache: MISS');
    }
...
if (is_string($notamCacheFile) && $notamCacheFile !== '') {
    notamMapCacheWrite($notamCacheFile, $payload);
}
...
if (is_array($notamPerf) && $notamPerfStart !== null) {
    ...
    header('X-YC-Notam-Map-Timing: ' . implode(',', $compactTiming));
}
